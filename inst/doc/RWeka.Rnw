...
loading, using, e.g.,
\verb|init = make_Weka_package_loader("lazyBayesianRules"))| for the
above registration.  The new mechanism is preferred as it provides
explicit information about the external package dependencies.)
